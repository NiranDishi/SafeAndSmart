...
def detectDrowning(source):
    ...
    while(cap.isOpened()):
        ...
        if len(bbox) == 1:
            ...
            if counter==65 and Alert==0:
                original_path = os.getcwd()  # Store the original working directory
                project_path = "../"
                os.chdir(project_path)
                subprocess.run(["python3", "Alert_Notification.py"])
            ...
            if counter==40:
                ...
                project_path = "../"
                os.chdir(project_path)
                subprocess.run(["python3", "Warning.py"])
                os.chdir(original_path)  # Return to the original working directory
                Alert=1
                print('-------------------------------')
        ...
        cv2.moveWindow("Real-time object detection", 50, 50)  # Replace x and y with desired coordinates
        cv2.namedWindow("Real-time object detection", cv2.WINDOW_NORMAL)

        cv2.resizeWindow("Real-time object detection", 450, 800)
        # press "Q" to stop
